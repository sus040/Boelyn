...
parser.add_argument('date', type=str, help='Date of the quality data (YYYY-MM-DD)')
        ...
        with psycopg2.connect(
            ...
            with conn.cursor() as cursor:
                ...
                if result and result[0]:
                    print("Table 'quality' exists. Proceeding with data insertion.")
                else:
                ...
                for _, row in quality_data.iterrows():
                    cursor.execute(
                        "INSERT INTO \quality (Facility_ID, hospital_type, hospital_ownership, emergency_services, quality_rating, rating_date) VALUES (%s, %s, %s, %s, %s, %s)",
                        (row['Facility ID'], row['Hospital Type'], row['Hospital Ownership'],row['Emergency Services'], row['Hospital overall rating'], datetime.strptime(date, '%Y-%m-%d')))
